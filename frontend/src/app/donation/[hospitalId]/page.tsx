...
export default function HospitalDetailPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/hospitals/${encodeURIComponent(hospitalId)}/medicines`, {
          cache: "no-store",
        });
  ...
  const onParcel = (m: Shortage) => {
    console.log("[donation] Parcel clicked:", m);
    // TODO: open a form / navigate to a parcel flow
  };

  const onMoney = (m: Shortage) => {
    console.log("[donation] Money clicked:", m);
    // TODO: open a payment flow
  };
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <header className="mt-2 mb-4">
          ...
          <p className="text-gray-700 mt-2">
            Requested medicines and current availability.
          </p>
        </header>
        ...
        {!loaded && !errMsg && (
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {Array.from({ length: 6 }).map((_, i) => (
              <div
                key={i}
                className="animate-pulse px-4 py-3 rounded-xl bg-white border border-stone-200"
              >
          ...
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {items.length === 0 && (
              <div className="text-sm text-gray-600">
                No medicine requests for this hospital yet.
              </div>
            )}

            {items.map((m) => (
              <div
                key={m._id ?? m.medicine}
                className="p-4 rounded-xl bg-white shadow-sm border border-stone-200"
              >
                ...
                <div className="mt-4 grid grid-cols-2 gap-2">
                  ...
                  <button
                    onClick={() => onMoney(m)}
                    className="px-3 py-2 rounded-lg border border-green-600 text-green-700 hover:bg-green-50 transition text-sm"
                  >
                    Money
                  </button>
                </div>
